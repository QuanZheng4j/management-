<template>
  <el-card class="main-card">
    <div style="display: flex">
      <div style="flex: 1;">
        <el-button type="primary" size="small" icon="el-icon-edit" @click="handleAddTeacher">添加部门</el-button>
      </div>
      <el-form :inline="true">
        <el-form-item>
          <el-input size="small" v-model="content" placeholder="按部门模糊搜索"></el-input>
        </el-form-item>
        <el-form-item>
...
<script>
import { selectDepartmentDTO, deleteOneDepartment } from '@/api/user'
export default {
  data() {
    return {
      total: 0, // 数据总长度
  ...
  methods: {
    ...
    getData() {
      ...
      console.log(data)
      selectDepartmentDTO(data).then(res => {
        console.log(res)
        if (res.data) {
    ...
    handleEditTeacher(row) {
      this.$router.push({path: 'updateDepart', query: row})
    },
    // 点击删除
    handleRemove(id, e) {
      this.$confirm('确定删除这个部门吗?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        console.log(id)
        deleteOneDepartment({id: id}).then(res => {
          console.log(res)
          if (res.data === '1') {
            this.$message({
              showClose: true,
